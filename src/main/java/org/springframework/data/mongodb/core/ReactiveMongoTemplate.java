...
public class ReactiveMongoTemplate implements ReactiveMongoOperations, ApplicationContextAware {
    ...
    public ReactiveMongoTemplate(ReactiveMongoDatabaseFactory mongoDatabaseFactory, @Nullable MongoConverter mongoConverter, Consumer<Throwable> subscriptionExceptionHandler) {
        ...
        this.queryOperations = new QueryOperations(queryMapper, updateMapper, operations, propertyOperations, mongoDatabaseFactory);
    ...
    private void onCheckForIndexes(MongoPersistentEntity<?> entity, Consumer<Throwable> subscriptionExceptionHandler) {
        ...
        if (indexCreator != null) {
            indexCreator.checkForIndexes(entity)
                    .subscribe(v -> {
    ...
    private void useEstimatedCount(boolean enabled, BiFunction<Document, CountOptions, Mono<Boolean>> estimationFilter) {
        ...
        if (enabled) {
            ...
            this.countExecution = (collectionName, filter, options) -> {
                ...
                return estimationFilter.apply(filter, options)
                        .flatMap(canEstimate -> {
                            if (!canEstimate) {
    ...
    private void prepareIndexCreator(ApplicationContext context) {
        ...
        for (String creator : indexCreators) {
            ReactiveMongoPersistentEntityIndexCreator creatorBean = context.getBean(creator, ReactiveMongoPersistentEntityIndexCreator.class);
    ...
    public Mono<Document> executeCommand(Document command, @Nullable ReadPreference readPreference) {
        ...
        return createFlux(db -> readPreference != null ? db.runCommand(command, readPreference, Document.class) : db.runCommand(command, Document.class)).next();
    }
    ...
    public ReactiveSessionScoped withSession(Publisher<ClientSession> sessionProvider) {
        ...
        Mono<ClientSession> cachedSession = Mono.from(sessionProvider)
                .cache();
    ...
    private <T> Flux<T> withSession(ReactiveSessionCallback<T> action, ClientSession session) {
        ...
        ReactiveSessionBoundMongoTemplate operations = new ReactiveSessionBoundMongoTemplate(session, ReactiveMongoTemplate.this);
    ...
    public <T> Flux<T> createFlux(ReactiveDatabaseCallback<T> callback) {
        ...
        return Mono.defer(this::doGetDatabase)
                .flatMapMany(database -> callback.doInDB(prepareDatabase(database)))
    ...
    public <T> Mono<T> createMono(ReactiveDatabaseCallback<T> callback) {
        ...
        return Mono.defer(this::doGetDatabase)
                .flatMap(database -> Mono.from(callback.doInDB(prepareDatabase(database))))
                .onErrorMap(translateException());
    }
    ...
    public <T> Flux<T> createFlux(String collectionName, ReactiveCollectionCallback<T> callback) {
        ...
        Mono<MongoCollection<Document>> collectionPublisher = doGetDatabase().map(database -> getAndPrepareCollection(database, collectionName));

        return collectionPublisher.flatMapMany(callback::doInCollection)
                .onErrorMap(translateException());
    }
    ...
    public <T> Mono<MongoCollection<Document>> createCollection(Class<T> entityClass) {
        return createCollection(entityClass, operations.forType(entityClass)
    ...
    public <T> Mono<MongoCollection<Document>> createCollection(Class<T> entityClass, @Nullable CollectionOptions collectionOptions) {
        ...
        options = Optionals.firstNonEmpty(() -> Optional.ofNullable(collectionOptions)
                        .flatMap(CollectionOptions::getCollation), () -> operations.forType(entityClass)
                        .getCollation()) //
    ...
    public Mono<MongoCollection<Document>> createView(String name, Class<?> source, AggregationPipeline pipeline, @Nullable ViewOptions options) {
        ...
        return createView(name, getCollectionName(source), queryOperations.createAggregation(Aggregation.newAggregation(source, pipeline.getOperations()), source), options);
    }

    @Override
    public Mono<MongoCollection<Document>> createView(String name, String source, AggregationPipeline pipeline, @Nullable ViewOptions options) {

        return createView(name, source, queryOperations.createAggregation(Aggregation.newAggregation(pipeline.getOperations()), (Class<?>) null), options);
    }

    private Mono<MongoCollection<Document>> createView(String name, String source, AggregationDefinition aggregation, @Nullable ViewOptions options) {
        return doCreateView(name, source, aggregation.getAggregationPipeline(), options);
    }

    protected Mono<MongoCollection<Document>> doCreateView(String name, String source, List<Document> pipeline, @Nullable ViewOptions options) {

        CreateViewOptions viewOptions = new CreateViewOptions();
        if (options != null) {
            options.getCollation()
                    .map(Collation::toMongoCollation)
                    .ifPresent(viewOptions::collation);
        }
    ...
    public ReactiveBulkOperations bulkOps(BulkMode mode, @Nullable Class<?> entityType, String collectionName) {
        ...
        DefaultReactiveBulkOperations operations = new DefaultReactiveBulkOperations(this, collectionName, new ReactiveBulkOperationContext(mode, Optional.ofNullable(getPersistentEntity(entityType)), queryMapper, updateMapper, eventPublisher, entityCallbacks));
    ...
    public <T> Mono<T> findOne(Query query, Class<T> entityClass, String collectionName) {
        ...
        if (ObjectUtils.isEmpty(query.getSortObject())) {
            return doFindOne(collectionName, ReactiveCollectionPreparerDelegate.of(query), query.getQueryObject(), query.getFieldsObject(), entityClass, new QueryFindPublisherPreparer(query, entityClass));
        }
    ...
    public Mono<Boolean> exists(Query query, @Nullable Class<?> entityClass, String collectionName) {
        ...
        return createFlux(collectionName, collection -> {
            ...
            FindPublisher<Document> findPublisher = collectionPreparer.prepare(collection)
    ...
    public <T> Flux<T> find(@Nullable Query query, Class<T> entityClass, String collectionName) {
        ...
        return doFind(collectionName, ReactiveCollectionPreparerDelegate.of(query), query.getQueryObject(), query.getFieldsObject(), entityClass, new QueryFindPublisherPreparer(query, entityClass));
    }
    ...
    <T> Mono<Window<T>> doScroll(Query query, Class<?> sourceClass, Class<T> targetClass, String collectionName) {
        ...
        if (query.hasKeyset()) {
            ...
            KeysetScrollQuery keysetPaginationQuery = ScrollUtils.createKeysetPaginationQuery(query, operations.getIdPropertyName(sourceClass));

            Mono<List<T>> result = doFind(collectionName, ReactiveCollectionPreparerDelegate.of(query), keysetPaginationQuery.query(), keysetPaginationQuery.fields(), sourceClass, new QueryFindPublisherPreparer(query, keysetPaginationQuery.sort(), limit, 0, sourceClass), callback).collectList();

            return result.map(it -> ScrollUtils.createWindow(query, it, sourceClass, operations));
        }

        Mono<List<T>> result = doFind(collectionName, ReactiveCollectionPreparerDelegate.of(query), query.getQueryObject(), query.getFieldsObject(), sourceClass, new QueryFindPublisherPreparer(query, query.getSortObject(), limit, query.getSkip(), sourceClass), callback).collectList();

        return result.map(it -> ScrollUtils.createWindow(it, query.getLimit(), OffsetScrollPosition.positionFunction(query.getSkip())));
    }
    ...
    public <T> Mono<T> findById(Object id, Class<T> entityClass, String collectionName) {
        ...
        return doFindOne(collectionName, CollectionPreparer.identity(), new Document(idKey, id), null, entityClass, (Collation) null);
    }
    ...
    public <T> Flux<T> findDistinct(Query query, String field, String collectionName, Class<?> entityClass, Class<T> resultClass) {
        ...
        Flux<?> result = execute(collectionName, collection -> {
            ...
            DistinctPublisher<T> publisher = collectionPreparer.prepare(collection)
    ...
    private <O> Flux<O> aggregateAndMap(MongoCollection<Document> collection, List<Document> pipeline, boolean isOutOrMerge, AggregationOptions options, ReadDocumentCallback<O> readCallback, @Nullable Class<?> inputType) {

        ReactiveCollectionPreparerDelegate collectionPreparer = ReactiveCollectionPreparerDelegate.of(options);
        AggregatePublisher<Document> cursor = collectionPreparer.prepare(collection)
        ...
        options.getComment()
                .ifPresent(cursor::comment);

        HintFunction hintFunction = options.getHintObject()
                .map(HintFunction::from)
                .orElseGet(HintFunction::empty);
        if (hintFunction.isPresent()) {
            cursor = hintFunction.apply(mongoDatabaseFactory, cursor::hintString, cursor::hint);
        }

        Optionals.firstNonEmpty(options::getCollation, () -> operations.forType(inputType)
                        .getCollation()) //
                .map(Collation::toMongoCollation) //
                .ifPresent(cursor::collation);

        if (options.hasExecutionTimeLimit()) {
            cursor = cursor.maxTime(options.getMaxTime()
    ...
    protected <T> Flux<GeoResult<T>> geoNear(NearQuery near, Class<?> entityClass, String collectionName, Class<T> returnType) {
        ...
        GeoNearResultDocumentCallback<T> callback = new GeoNearResultDocumentCallback<>(distanceField, new ProjectingReadCallback<>(mongoConverter, projection, collection), near.getMetric());
    ...
    public <T> Mono<T> findAndModify(Query query, UpdateDefinition update, FindAndModifyOptions options, Class<T> entityClass, String collectionName) {
        ...
        return doFindAndModify(collectionName, ReactiveCollectionPreparerDelegate.of(query), query.getQueryObject(), query.getFieldsObject(), getMappedSortObject(query, entityClass), entityClass, update, optionsToUse);
    }
    ...
    public <S, T> Mono<T> findAndReplace(Query query, S replacement, FindAndReplaceOptions options, Class<S> entityType, String collectionName, Class<T> resultType) {
        ...
        return Mono.defer(() -> {
            ...
            return maybeCallBeforeConvert(pem.getSource(), pem.getCollection()).map(pem::mutate)
                    .flatMap(it -> {
                        PersistableEntityModel<S> mapped = it.addTargetDocument(operations.forEntity(it.getSource())
                                .toMappedDocument(mongoConverter)
                                .getDocument());
                    ...
                    .flatMap(it -> {
                        ...
                        Mono<T> afterFindAndReplace = doFindAndReplace(it.getCollection(), collectionPreparer, mappedQuery, mappedFields, mappedSort, queryContext.getCollation(entityType)
                                .orElse(null), entityType, it.getTarget(), options, projection);
    ...
    public <T> Mono<T> findAndRemove(Query query, Class<T> entityClass, String collectionName) {
        ...
        operations.forType(entityClass)
                .getCollation(query);
        return doFindAndRemove(collectionName, ReactiveCollectionPreparerDelegate.of(query), query.getQueryObject(), query.getFieldsObject(), getMappedSortObject(query, entityClass), operations.forType(entityClass)
                .getCollation(query)
                .orElse(null), entityClass);
    }
    ...
    protected Mono<Long> doExactCount(String collectionName, Document filter, CountOptions options) {
        ...
        return createMono(collectionName, collection -> collection.countDocuments(CountQuery.of(filter)
                .toQueryDocument(), options));
    }
    ...
    protected <T> Mono<T> doInsert(String collectionName, T objectToSave, MongoWriter<Object> writer) {
        ...
        return Mono.just(PersistableEntityModel.of(objectToSave, collectionName)) //
                ...
                .map(it -> {
                    ...
                    PersistableEntityModel<T> model = PersistableEntityModel.of(entity.initializeVersionProperty(), entity.toMappedDocument(writer)
                            .getDocument(), it.getCollection());
                ...
                .flatMap(it -> {
                    ...
                    return insertDocument(it.getCollection(), it.getTarget(), it.getSource()
                            .getClass()).flatMap(id -> {
    ...
    public <T> Flux<T> insertAll(Mono<? extends Collection<? extends T>> objectsToSave) {
        return Flux.from(objectsToSave)
    ...
    protected <T> Flux<T> doInsertBatch(String collectionName, Collection<? extends T> batchToSave, MongoWriter<Object> writer) {
        ...
        Mono<List<Tuple2<AdaptibleEntity<T>, Document>>> prepareDocuments = Flux.fromIterable(batchToSave)
                .flatMap(uninitialized -> {
                    ...
                    return maybeCallBeforeConvert(toConvert, collectionName).flatMap(it -> {
                        ...
                        Document dbDoc = entity.toMappedDocument(writer)
                                .getDocument();
        ...
        Flux<Tuple2<AdaptibleEntity<T>, Document>> insertDocuments = prepareDocuments.flatMapMany(tuples -> {
            ...
            List<Document> documents = tuples.stream()
                    .map(Tuple2::getT2)
        ...
        return insertDocuments.flatMap(tuple -> {
            ...
            Object id = MappedDocument.of(document)
                    .getId();
    ...
    public <T> Mono<T> save(T objectToSave, String collectionName) {
        ...
        return source.isVersionedEntity() ? doSaveVersioned(source, collectionName) : doSave(collectionName, objectToSave, this.mongoConverter);
    }
    ...
    private <T> Mono<T> doSaveVersioned(AdaptibleEntity<T> source, String collectionName) {
        ...
        return createMono(collectionName, collection -> {
            ...
            return maybeCallBeforeConvert(afterEvent, collectionName).flatMap(toConvert -> {
                ...
                MappedDocument mapped = operations.forEntity(toConvert)
                        .toMappedDocument(mongoConverter);
                ...
                return maybeCallBeforeSave(toConvert, document, collectionName).flatMap(it -> {
                    ...
                    return doUpdate(collectionName, query, mapped.updateWithoutId(), it.getClass(), false, false).flatMap(result -> {
    ...
    protected Mono<Object> insertDocument(String collectionName, Document dbDoc, Class<?> entityClass) {
        ...
        queryOperations.createInsertContext(document)
    ...
    protected Mono<Object> saveDocument(String collectionName, Document document, Class<?> entityClass) {
        ...
        return createMono(collectionName, collection -> {
            ...
            Publisher<?> publisher;
            if (!mapped.hasId()) {
                publisher = collectionToUse.insertOne(queryOperations.createInsertContext(mapped)
                        .prepareId(entityClass)
                        .getDocument());
            } else {
                ...
                if (updateContext.requiresShardKey(filter, entity)) {
                    if (entity.getShardKey()
                            .isImmutable()) {
                        deferredFilter = Mono.just(updateContext.applyShardKey(entity, filter, null));
                    } else {
                        deferredFilter = Mono.from(collection.find(filter, Document.class)
                                        .projection(updateContext.getMappedShardKey(entity))
                                        .first())
                                .defaultIfEmpty(replacement)
                                .map(it -> updateContext.applyShardKey(entity, filter, it));
                    }
                } else {
    ...
    private boolean containsVersionProperty(Document document, @Nullable MongoPersistentEntity<?> persistentEntity) {
        ...
        return document.containsKey(persistentEntity.getRequiredVersionProperty()
                .getFieldName());
    }
    ...
    public Mono<DeleteResult> remove(Object object) {
        ...
        return remove(operations.forEntity(object)
                .getRemoveByQuery(), object.getClass());
    }
    ...
    public Mono<DeleteResult> remove(Object object, String collectionName) {
        ...
        return doRemove(collectionName, operations.forEntity(object)
                .getRemoveByQuery(), object.getClass());
    }
    ...
    private void assertUpdateableIdIfNotSet(Object value) {
        ...
        if (entity != null && entity.hasIdProperty()) {
            ...
            Object propertyValue = entity.getPropertyAccessor(value)
            ...
            if (!MongoSimpleTypes.AUTOGENERATED_ID_TYPES.contains(property.getType())) {
                throw new InvalidDataAccessApiUsageException(String.format("Cannot autogenerate id of type %s for entity of type %s", property.getType()
                        .getName(), value.getClass()
                        .getName()));
            }
        }
    }
    ...
    protected <T> Mono<DeleteResult> doRemove(String collectionName, Query query, @Nullable Class<T> entityClass) {
        ...
        return execute(collectionName, collection -> {
            ...
            Mono<Void> aggregateQuery = removeQuery.get(AGGREGATE_PUBLISHER, Mono.empty()
                            .then())
                    .doOnSuccess(unused -> removeQuery.remove(AGGREGATE_PUBLISHER));

            MongoCollection<Document> collectionToUse = collectionPreparer.prepare(prepareCollection(collection, writeConcernToUse));
            ...
            if (query.getLimit() > 0 || query.getSkip() > 0) {
                ...
                FindPublisher<Document> cursor = new QueryFindPublisherPreparer(query, entityClass).prepare(collection.find(removeQuery)) //
                ...
                return Flux.from(cursor) //
                        ...
                        .collectList() //
                        .flatMapMany(val -> aggregateQuery.thenMany(collectionToUse.deleteMany(MappedDocument.getIdIn(val), deleteOptions)));
            } else {
                return aggregateQuery.thenMany(collectionToUse.deleteMany(removeQuery, deleteOptions));
            }
    ...
    public <T> Flux<T> findAll(Class<T> entityClass, String collectionName) {
        return executeFindMultiInternal(new FindCallback(CollectionPreparer.identity(), null), FindPublisherPreparer.NO_OP_PREPARER, new ReadDocumentCallback<>(mongoConverter, entityClass, collectionName), collectionName);
    }
    ...
    public <T> Flux<T> tail(@Nullable Query query, Class<T> entityClass, String collectionName) {
        ...
        if (query == null) {
            ...
            return executeFindMultiInternal(collection -> new FindCallback(collectionPreparer, null).doInCollection(collection)
                    .cursorType(CursorType.TailableAwait), FindPublisherPreparer.NO_OP_PREPARER, new ReadDocumentCallback<>(mongoConverter, entityClass, collectionName), collectionName);
        }

        return doFind(collectionName, collectionPreparer, query.getQueryObject(), query.getFieldsObject(), entityClass, new TailingQueryFindPublisherPreparer(query, entityClass));
    }
        ...
        return ReactiveMongoDatabaseUtils.getDatabase(database, mongoDatabaseFactory) //
                .map(db -> {
                    ...
                    if (StringUtils.hasText(collectionName)) {
                        publisher = filter.isEmpty() ? db.getCollection(collectionName)
                                .watch(Document.class) : db.getCollection(collectionName)
                                .watch(filter, Document.class);
                    ...
                    if (options.isResumeAfter()) {
                        publisher = options.getResumeToken()
                                .map(BsonValue::asDocument)
                                .map(publisher::resumeAfter)
                                .orElse(publisher);
                    } else if (options.isStartAfter()) {
                        publisher = options.getResumeToken()
                                .map(BsonValue::asDocument)
                                .map(publisher::startAfter)
                                .orElse(publisher);
                    }
                    publisher = options.getCollation()
                            .map(Collation::toMongoCollation)
                            .map(publisher::collation)
                            .orElse(publisher);
                    publisher = options.getResumeBsonTimestamp()
                            .map(publisher::startAtOperationTime)
                            .orElse(publisher);
                    return publisher.fullDocument(options.getFullDocumentLookup()
                            .orElse(fullDocument));
                }) //
    ...
    List<Document> prepareFilter(ChangeStreamOptions options) {
        ...
        Object filter = options.getFilter()
                .orElse(Collections.emptyList());

        if (filter instanceof Aggregation agg) {
            AggregationOperationContext context = agg instanceof TypedAggregation typedAggregation ? new TypeBasedAggregationOperationContext(typedAggregation.getInputType(), getConverter().getMappingContext(), queryMapper) : new RelaxedTypeBasedAggregationOperationContext(Object.class, mappingContext, queryMapper);
    ...
    public <T> Flux<T> mapReduce(Query filterQuery, Class<?> domainType, Class<T> resultType, String mapFunction, String reduceFunction, MapReduceOptions options) {
        ...
        return mapReduce(filterQuery, domainType, getCollectionName(domainType), resultType, mapFunction, reduceFunction, options);
    }
    ...
    public <T> Flux<T> mapReduce(Query filterQuery, Class<?> domainType, String inputCollectionName, Class<T> resultType, String mapFunction, String reduceFunction, MapReduceOptions options) {
        ...
        return createFlux(inputCollectionName, collection -> {
            ...
            MapReducePublisher<Document> publisher = collectionPreparer.prepare(collection)
                    .mapReduce(mapFunction, reduceFunction, Document.class);
            ...
            if (options.getFinalizeFunction()
                    .filter(StringUtils::hasText)
                    .isPresent()) {
                publisher = publisher.finalizeFunction(options.getFinalizeFunction()
                        .get());
            }
            ...
            if (options.getOutputSharded()
                    .isPresent()) {
                publisher = publisher.sharded(options.getOutputSharded()
                        .get());
            }
            ...
            if (StringUtils.hasText(options.getOutputCollection()) && !options.usesInlineOutput()) {
                publisher = publisher.collectionName(options.getOutputCollection())
                        .action(options.getMapReduceAction());

                if (options.getOutputDatabase()
                        .isPresent()) {
                    publisher = publisher.databaseName(options.getOutputDatabase()
                            .get());
                }
            }

            publisher = collation.map(Collation::toMongoCollation)
                    .map(publisher::collation)
                    .orElse(publisher);
     ...
     */
    protected <T> Mono<T> doFindOne(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, @Nullable Document fields, Class<T> entityClass, @Nullable Collation collation) {

        return doFindOne(collectionName, collectionPreparer, query, fields, entityClass, findPublisher -> collation != null ? findPublisher.collation(collation.toMongoCollation()) : findPublisher);
    }
     ...
     * @since 2.2
     */
    protected <T> Mono<T> doFindOne(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, @Nullable Document fields, Class<T> entityClass, FindPublisherPreparer preparer) {
        ...
        return executeFindOneInternal(new FindOneCallback(collectionPreparer, mappedQuery, mappedFields, preparer), new ReadDocumentCallback<>(this.mongoConverter, entityClass, collectionName), collectionName);
    }
     ...
     */
    protected <T> Flux<T> doFind(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, Document fields, Class<T> entityClass) {
        return doFind(collectionName, collectionPreparer, query, fields, entityClass, null, new ReadDocumentCallback<>(this.mongoConverter, entityClass, collectionName));
    }
     ...
     * @return the {@link List} of converted objects.
     */
    protected <T> Flux<T> doFind(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, Document fields, Class<T> entityClass, FindPublisherPreparer preparer) {
        return doFind(collectionName, collectionPreparer, query, fields, entityClass, preparer, new ReadDocumentCallback<>(mongoConverter, entityClass, collectionName));
    }

    protected <S, T> Flux<T> doFind(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, Document fields, Class<S> entityClass, @Nullable FindPublisherPreparer preparer, DocumentCallback<T> objectCallback) {
        ...
        return executeFindMultiInternal(new FindCallback(collectionPreparer, mappedQuery, mappedFields), preparer, objectCallback, collectionName);
    }
     ...
     * @since 2.0
     */
    <S, T> Flux<T> doFind(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, Document fields, Class<S> sourceClass, Class<T> targetClass, FindPublisherPreparer preparer) {
        ...
        return executeFindMultiInternal(new FindCallback(collectionPreparer, mappedQuery, mappedFields), preparer, new ProjectingReadCallback<>(mongoConverter, projection, collectionName), collectionName);
    }
     ...
     * @return the List of converted objects.
     */
    protected <T> Mono<T> doFindAndRemove(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, Document fields, Document sort, @Nullable Collation collation, Class<T> entityClass) {

        if (LOGGER.isDebugEnabled()) {
            LOGGER.debug(String.format("findAndRemove using query: %s fields: %s sort: %s for class: %s in collection: %s", serializeToJsonSafely(query), fields, sort, entityClass, collectionName));
        }

        MongoPersistentEntity<?> entity = mappingContext.getPersistentEntity(entityClass);

        return executeFindOneInternal(new FindAndRemoveCallback(collectionPreparer, queryMapper.getMappedObject(query, entity), fields, sort, collation), new ReadDocumentCallback<>(this.mongoConverter, entityClass, collectionName), collectionName);
    }

    protected <T> Mono<T> doFindAndModify(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, Document fields, Document sort, Class<T> entityClass, UpdateDefinition update, FindAndModifyOptions options) {
        ...
        return Mono.defer(() -> {
            ...
            return executeFindOneInternal(new FindAndModifyCallback(collectionPreparer, mappedQuery, fields, sort, mappedUpdate, update.getArrayFilters()
                    .stream()
                    .map(ArrayFilter::asDocument)
                    .collect(Collectors.toList()), options), new ReadDocumentCallback<>(this.mongoConverter, entityClass, collectionName), collectionName);
        });
    }
    ...
    protected <T> Mono<T> doFindAndReplace(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document mappedQuery, Document mappedFields, Document mappedSort, com.mongodb.client.model.Collation collation, Class<?> entityType, Document replacement, FindAndReplaceOptions options, Class<T> resultType) {
        ...
        return doFindAndReplace(collectionName, collectionPreparer, mappedQuery, mappedFields, mappedSort, collation, entityType, replacement, options, projection);
    }
    ...
    private <T> Mono<T> doFindAndReplace(String collectionName, CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document mappedQuery, Document mappedFields, Document mappedSort, com.mongodb.client.model.Collation collation, Class<?> entityType, Document replacement, FindAndReplaceOptions options, EntityProjection<T, ?> projection) {
        ...
        return Mono.defer(() -> {
            ...
            if (LOGGER.isDebugEnabled()) {
                LOGGER.debug(String.format("findAndReplace using query: %s fields: %s sort: %s for class: %s and replacement: %s " + "in collection: %s", serializeToJsonSafely(mappedQuery), mappedFields, mappedSort, entityType, serializeToJsonSafely(replacement), collectionName));
            }

            return executeFindOneInternal(new FindAndReplaceCallback(collectionPreparer, mappedQuery, mappedFields, mappedSort, replacement, collation, options), new ProjectingReadCallback<>(this.mongoConverter, projection, collectionName), collectionName);
    ...
    private WriteConcern potentiallyForceAcknowledgedWrite(@Nullable WriteConcern wc) {
        ...
        if (ObjectUtils.nullSafeEquals(WriteResultChecking.EXCEPTION, writeResultChecking)) {
            if (wc == null || wc.getWObject() == null || (wc.getWObject() instanceof Number concern && concern.intValue() < 1)) {
    ...
    private <T> Mono<T> executeFindOneInternal(ReactiveCollectionCallback<Document> collectionCallback, DocumentCallback<T> objectCallback, String collectionName) {
        ...
        return createMono(collectionName, collection -> Mono.from(collectionCallback.doInCollection(collection))
                .flatMap(objectCallback::doWith));
    }
    ...
    private static class FindOneCallback implements ReactiveCollectionCallback<Document> {
        ...
        FindOneCallback(CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, @Nullable Document fields, FindPublisherPreparer preparer) {
        ...
        public Publisher<Document> doInCollection(MongoCollection<Document> collection) throws MongoException, DataAccessException {
            ...
            FindPublisher<Document> publisher = preparer.initiateFind(collectionPreparer.prepare(collection), col -> col.find(query, Document.class));

            if (fields.isPresent()) {
                publisher = publisher.projection(fields.get());
            }

            return publisher.limit(1)
                    .first();
        }
    }
    ...
    private static class FindAndRemoveCallback implements ReactiveCollectionCallback<Document> {
        ...
        public Publisher<Document> doInCollection(MongoCollection<Document> collection) throws MongoException, DataAccessException {
            ...
            collation.map(Collation::toMongoCollation)
                    .ifPresent(findOneAndDeleteOptions::collation);

            return collectionPreparer.prepare(collection)
                    .findOneAndDelete(query, findOneAndDeleteOptions);
        }
    }
    ...
    private static class FindAndModifyCallback implements ReactiveCollectionCallback<Document> {
        ...
        public Publisher<Document> doInCollection(MongoCollection<Document> collection) throws MongoException, DataAccessException {
            ...
            if (options.isRemove()) {
                ...
                findOneAndDeleteOptions = options.getCollation()
                        .map(Collation::toMongoCollation)
                        .map(findOneAndDeleteOptions::collation)
                        .orElse(findOneAndDeleteOptions);
            ...
            return Flux.error(new IllegalArgumentException(String.format("Using %s is not supported in findOneAndUpdate", update)));
        }
        ...
        private static FindOneAndUpdateOptions convertToFindOneAndUpdateOptions(FindAndModifyOptions options, Document fields, Document sort, List<Document> arrayFilters) {
            ...
            result = result.projection(fields)
                    .sort(sort)
            ...
            result = options.getCollation()
                    .map(Collation::toMongoCollation)
                    .map(result::collation)
                    .orElse(result);
    ...
    private static class FindAndReplaceCallback implements ReactiveCollectionCallback<Document> {
        ...
        FindAndReplaceCallback(CollectionPreparer<MongoCollection<Document>> collectionPreparer, Document query, Document fields, Document sort, Document update, com.mongodb.client.model.Collation collation, FindAndReplaceOptions options) {
            this.collectionPreparer = collectionPreparer;
            this.query = query;
            this.fields = fields;
            this.sort = sort;
        ...
        public Publisher<Document> doInCollection(MongoCollection<Document> collection) throws MongoException, DataAccessException {
            ...
            return collectionPreparer.prepare(collection)
                    .findOneAndReplace(query, update, findOneAndReplaceOptions);
        }
    ...
    private class ProjectingReadCallback<S, T> implements DocumentCallback<T> {
        ...
        public Mono<T> doWith(Document document) {
            ...
            Class<T> returnType = projection.getMappedType()
                    .getType();
    ...
    class QueryFindPublisherPreparer implements FindPublisherPreparer {
        ...
        public FindPublisher<Document> prepare(FindPublisher<Document> findPublisher) {
            ...
            try {
                ...
                if (hintFunction.isPresent()) {
                    findPublisherToUse = hintFunction.apply(mongoDatabaseFactory, findPublisherToUse::hintString, findPublisherToUse::hint);
                }
